...
/*
 * 修订记录:
 * turalyon@jinvovo.com 2017-03-19 09:08 创建
 *
 */
 ...
 */
public class JvvRedisConnectionFactory implements InitializingBean, DisposableBean, RedisConnectionFactory {

	private final static Log log = LogFactory.getLog(JvvRedisConnectionFactory.class);
	...
	/**
	 * Constructs a new <code>YedisConnectionFactory</code> instance with default settings (default connection pooling,
	 * no shard information).
	 */
	public JvvRedisConnectionFactory() {
	}

	/**
	 * Constructs a new <code>YedisConnectionFactory</code> instance. Will override the other connection parameters
	 * passed to the factory.
	 *
	 * @param shardInfo shard information
	 */
	public JvvRedisConnectionFactory(JedisShardInfo shardInfo) {
		this.shardInfo = shardInfo;

	}

	/**
	 * Constructs a new <code>YedisConnectionFactory</code> instance using the given pool configuration.
	 *
	 * @param poolConfig pool configuration
	 */
	public JvvRedisConnectionFactory(JedisPoolConfig poolConfig) {
		this(null, poolConfig);
	}

	/**
	 * Constructs a new {@link JvvRedisConnectionFactory} instance using the given {@link JedisPoolConfig} applied to
	 * {@link redis.clients.jedis.JedisSentinelPool}.
	 *
	 * @param sentinelConfig
	 * @since 1.4
	 */
	public JvvRedisConnectionFactory(RedisSentinelConfiguration sentinelConfig) {
	 ...
	 * @since 1.4
	 */
	public JvvRedisConnectionFactory(RedisSentinelConfiguration sentinelConfig, JedisPoolConfig poolConfig) {
	...
	public JedisConnection getConnection() {
		...
		JvvRedisConnection connection = (usePool ? new JvvRedisConnection(jedis, pool, dbIndex, namespace)
				: new JvvRedisConnection(jedis, null, dbIndex, namespace));
	...
	public JedisConnection getConnectionWithoutNS() {
		Jedis jedis = fetchJedisConnector();
		JvvRedisConnection connection = (usePool ? new JvvRedisConnection(jedis, pool, dbIndex)
				: new JvvRedisConnection(jedis, null, dbIndex));
		connection.setConvertPipelineAndTxResults(convertPipelineAndTxResults);
		return postProcessConnection(connection);
	}
