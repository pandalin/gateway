...
/*
 * 修订记录:
 * turalyon@jinvovo.com 2017-03-19 09:10 创建
 *
 */
 ...
 */
public class JvvRedisConnection extends JedisConnection {
	...
	public JvvRedisConnection(Jedis jedis, String namespace) {
		super(jedis);
		if (null == namespace) {
			throw new RedisException("namespace must be set.");
		}
		this.namespace = namespace.getBytes();

	}

	public JvvRedisConnection(Jedis jedis, Pool<Jedis> pool, int dbIndex) {
		super(jedis, pool, dbIndex);
	}

	public JvvRedisConnection(Jedis jedis, Pool<Jedis> pool, int dbIndex, String namespace) {
		super(jedis, pool, dbIndex);
		if (null == namespace) {
			throw new RedisException("namespace must be set.");
		}
	...
	private byte[] bindNamespace(byte[] rawKey) {
		...
		try (ByteArrayOutputStream writer = buffer.get()) {
			...
			throw new RedisException("bindNamespace failed.", e);
		}
	}
	...
	public Object execute(String command, byte[]... args) {
		throw new JvvRedisUnSupportCommandException();
	}
